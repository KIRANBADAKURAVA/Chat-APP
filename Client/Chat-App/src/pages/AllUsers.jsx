...
function AllUsers() {
  ...
  const handleChatClick = async (user) => {
    ...
    try {
      const response = await fetch(`https://chat-app-backend-ezj4.onrender.com/api/v1/message/sendIndividualMessage/${user._id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleSearch = async (term) => {
    try {
      const response = await fetch(`https://chat-app-backend-ezj4.onrender.com/api/v1/user/searchuser?username=${term}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
